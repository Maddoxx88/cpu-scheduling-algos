<script lang="ts">
    ...
    type Algorithm = 'fcfs' | 'sjf' | 'ljf' | 'hrrn' | 'multilevel';
    ...
    let nextId = 4;
    let selectedAlgorithm: Algorithm = 'fcfs';
    let currentTime = 0;
    ...
    function findNextProcess(
      ...
      switch (algorithm) {
        case 'fcfs':
          // Return the process with earliest arrival time
          return availableProcesses.reduce((earliest, p) => 
    ...
    function calculateSchedule(): void {
      ...
      // Pre-sort by arrival time for FCFS
