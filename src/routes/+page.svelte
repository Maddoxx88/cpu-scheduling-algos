 
  <main class="p-4">
    <h1 class="text-2xl font-bold mb-2">Pre-emptive Algos</h1>
    <div class="flex mb-4">
      <h1>SRTF&nbsp;-&nbsp;</h1>
      <a href="/fcfs" class="text-blue-500 font-mono">/srtf</a>
    </div>
    <div class="flex mb-4">
      <h1>LRTF&nbsp;-&nbsp;</h1>
      <a href="/fcfs" class="text-blue-500 font-mono">/lrtf</a>
    </div>
    <div class="flex mb-4">
      <h1>Round Robin&nbsp;-&nbsp;</h1>
      <a href="/fcfs" class="text-blue-500 font-mono">/rr</a>
    </div>
    <div class="flex mb-8">
      <h1>Priority based&nbsp;-&nbsp;</h1>
      <a href="/priority-based" class="text-blue-500 font-mono">/priority-based</a>
    </div>

    <h1 class="text-2xl font-bold mb-2">Non pre-emptive Algos</h1>
    <div class="flex mb-4">
      <h1>FCFS&nbsp;-&nbsp;</h1>
      <a href="/fcfs" class="text-blue-500 font-mono">/fcfs</a>
    </div>
    <div class="flex mb-4">
      <h1>SJF&nbsp;-&nbsp;</h1>
      <a href="/sjf" class="text-blue-500 font-mono">/sjf</a>
    </div>
    <div class="flex mb-4">
      <h1>LJF&nbsp;-&nbsp;</h1>
      <a href="/non-prempt" class="text-blue-500 font-mono">/ljf</a>
    </div>
    <div class="flex mb-4">
      <h1>HRRN&nbsp;-&nbsp;</h1>
      <a href="/non-prempt" class="text-blue-500 font-mono">/hrrrn</a>
    </div>
    <div class="flex mb-4">
      <h1>Multi-level Queue&nbsp;-&nbsp;</h1>
      <a href="/non-prempt" class="text-blue-500 font-mono">/multiq</a>
    </div>
  </main>